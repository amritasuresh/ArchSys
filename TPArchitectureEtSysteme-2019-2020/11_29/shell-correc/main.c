...
{
	apply(cmd->input, STDIN_FILENO, O_RDONLY);
	apply(cmd->output, STDOUT_FILENO, O_WRONLY | O_CREAT | O_TRUNC);
	apply(cmd->append, STDOUT_FILENO, O_WRONLY | O_CREAT | O_APPEND);
	apply(cmd->error, STDERR_FILENO, O_WRONLY | O_CREAT | O_TRUNC);
}
